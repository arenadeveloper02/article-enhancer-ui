  ...
  return (
    <main className="min-h-screen w-full px-4 py-6 sm:px-6">
      {/* Shared content container: every major section (header, toggle,
          ...
          width — ~92% of the viewport capped at 1600px. Mobile stays 100%
          width (minus padding); the page is never full-bleed. */}
      <div className="mx-auto w-full max-w-[1600px]">
        {/* screen-only: hidden inside @media print so the exported PDF contains
            only the PrintableReport rendered by the active view. Compact header:
            reduced vertical spacing, centered title + description (max ~760px),
            32px gap down to the first card. */}
        <header className="screen-only mb-8 text-center">
          <h1 className="font-display text-3xl font-semibold tracking-tight text-ink sm:text-4xl">
            Article Enhancer Agent
          </h1>
          <p className="mx-auto mt-3 max-w-[760px] text-base leading-relaxed text-ink-soft">
